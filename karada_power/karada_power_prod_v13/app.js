...
/* ===== v13 production: D1 cloud sync =====
   Same-origin Worker route: /api/karada-power/*
   Local storage remains the instant/offline cache. D1 is the durable copy.
*/
...
function setCloudStatus(mode,msg=''){
  const title=document.getElementById('cloud-save-title');
  const detail=document.getElementById('cloud-save-detail');
  const btn=document.getElementById('cloud-pair-btn');
  if(!title||!detail||!btn)return;
  if(mode==='connected'){
    title.textContent='☁️ D1に保存中';
    detail.textContent=msg||'この端末の食事記録は、端末内＋D1の両方に残るよ。';
    btn.textContent='✓ 接続済み';
  }else if(mode==='error'){
    title.textContent='📱 端末には保存中';
    detail.textContent=msg||'D1に接続できなかったよ。端末内の記録はそのまま残っています。';
    btn.textContent='☁️ 再接続';
  }else{
    title.textContent='📱 この端末に保存中';
    detail.textContent=msg||'D1をつなぐと、アプリ更新後もクラウドに記録が残るよ。';
    btn.textContent='☁️ D1につなぐ';
  }
}
async function cloudRequest(path,options={}){
  const headers=cloudHeaders(options.headers||{});
  const res=await fetch(KP_CLOUD_BASE+path,{...options,headers});
...
async function cloudPushRecord(child,date,record){
  ...
  try{
    await cloudRequest(`/records/${encodeURIComponent(child)}/${date}`,{
      method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({record})
    });
    setCloudStatus('connected');
  }catch(e){setCloudStatus('error','D1への保存に失敗。端末内には保存済みです。');}
}
...
async function cloudPullAll(){
  ...
  try{
    ...
    (data.records||[]).forEach(row=>{
      const remote=normalizeRecord(row.record);
      const key=recordKey(row.child,row.record_date);
      let local=null;try{local=JSON.parse(localStorage.getItem(key)||'null')}catch(e){}
      // Most recently edited full-day record wins. Remote is authoritative on a fresh device.
      if(!local || String(remote.updatedAt||row.updated_at||'')>=String(local.updatedAt||'')){
        localStorage.setItem(key,JSON.stringify(remote));
      }else{
        cloudPushRecord(row.child,row.record_date,local);
      }
    });
    setCloudStatus('connected',`${(data.records||[]).length}日分の記録をD1と同期しました。`);
    renderToday();renderFoods();
  }catch(e){
    if(e.status===401){localStorage.removeItem(KP_DEVICE_TOKEN_KEY);setCloudStatus('error','D1の接続情報が無効です。もう一度つないでね。')}
    else setCloudStatus('error','D1に接続できません。端末内の記録は使えます。');
  }finally{kpCloudBusy=false}
}
async function cloudPair(){
  if(cloudToken()){
    await cloudPullAll();return;
  }
  const code=prompt('D1の家族コードを入力してね。\n※最初にこの端末を登録するときだけ使います。');
  if(!code)return;
  const label=prompt('この端末の名前（例：長女iPhone）','iPhone')||'family-device';
  try{
    const res=await fetch(KP_CLOUD_BASE+'/pair',{
      method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code,label})
    });
    const body=await res.json().catch(()=>({}));
    if(!res.ok)throw new Error(body.error||`HTTP ${res.status}`);
    localStorage.setItem(KP_DEVICE_TOKEN_KEY,body.token);
    localStorage.setItem(KP_DEVICE_LABEL_KEY,label);
    setCloudStatus('connected','接続できたよ。端末にある記録もD1へ同期します。');
    // Upload all local v12/v13 records first, then pull/merge.
    for(let i=0;i<localStorage.length;i++){
      const k=localStorage.key(i),m=k&&k.match(/^kp_record_v12_(.+)_(\\d{4}-\\d{2}-\\d{2})$/);
      if(m){let r=null;try{r=JSON.parse(localStorage.getItem(k))}catch(e){};if(r)await cloudPushRecord(m[1],m[2],r)}
    }
    await cloudPullAll();
